...
struct
  ...
  type cpuTime = {usr:Time.time, sys:Time.time}
  ...
  fun minus ({usr = t1, sys = t2},
	     {usr = s1, sys = s2}) =
      {usr = Time.-(t1,s1), sys = Time.-(t2,s2)}

  fun plus ({usr = t1, sys = t2},
	    {usr = s1, sys = s2}) =
      {usr = Time.+(t1,s1), sys = Time.+(t2,s2)}

  fun sum ({usr = t1, sys = t2}) = Time.+ (t1, t2)
  ...
  in
    ...
    *)
    fun time (_, counters) (f:'a -> 'b) (x:'a) =
	    ...
	    val evalCPUTime = Timer.checkCPUTimer (CPUTimer)
            val evalCPUTime = {usr=(#usr evalCPUTime), sys=(#sys evalCPUTime)}
    ...
    *)
    ...
    fun timesToString (name, (CPUTime as {usr = t1, sys = t2}, realTime)) =
        name ^ ": "
